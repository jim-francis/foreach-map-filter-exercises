...
function showFirstAndLast(arr) {
    ...
    arr.forEach(function (val) {
        let first = val[0]
        let last = val.slice(val.length - 1)
...
function extractFullName(arr) {
    const newArr = arr.map(function (vals) {
...
function findInObj(arr, key, searchValue) {
    let found = undefined;
    arr.filter(function (el) {
        if (found === undefined && el[key] === searchValue)
            found = el;
    })
    return found;
}
...
function removeVowels(str) {
    const arr = str.toLowerCase().split('');
    const vowels = 'aeiou';
    const newArr = arr.filter(function (char) {
        if (vowels.indexOf(char) === -1) {
            return char;
        }
    })
...
function doubleOddNumbers(arr) {
    ...
    const doubled = newArr.map(function (num) {
        return num * 2;
    })
    return doubled;
}
